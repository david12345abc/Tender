...
from gpb_inter_rao_client import (
    GPB_INTER_RAO_PROCEDURE_TYPE_OPTIONS,
...
from roseltorg_interrao_client import (
    ROSELTORG_INTERRAO_PROCEDURE_TYPE_OPTIONS,
...
from tektorg_inter_rao_client import (
...
from tektorg_rosneft_client import (
    ...
    TektorgRosneftClient,
)
from transneft_client import (
    TRANSNEFT_PROCEDURE_TYPE_OPTIONS,
...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self) -> None:
        ...
        self.act_platform_transneft = self.gpb_platform_menu.addAction("Газпром Транснефть")
        self.act_platform_gpb_inter_rao = self.gpb_platform_menu.addAction("Газпром Интер РАО")
        ...
        self.act_platform_roseltorg_223 = self.roseltorg_platform_menu.addAction("223-ФЗ")
        self.act_platform_roseltorg_interrao = self.roseltorg_platform_menu.addAction("Корпоративные закупки")
        ...
        self.btn_platform_tektorg.setFixedHeight(40)
        self.tektorg_platform_menu = QMenu(self.btn_platform_tektorg)
        self.act_platform_tektorg_rosneft = self.tektorg_platform_menu.addAction('НК "Роснефть"')
        ...
        self.act_platform_roseltorg_223.triggered.connect(lambda: self._select_platform("roseltorg_223"))
        self.act_platform_roseltorg_interrao.triggered.connect(lambda: self._select_platform("roseltorg_interrao"))
    ...
    def _platform_title(self) -> str:
        ...
        if self._platform_key == "roseltorg_223":
            return "Росэлторг 223-ФЗ"
        if self._platform_key == "roseltorg_interrao":
            return "Росэлторг — Корпоративные закупки"
        ...
        if self._platform_key == "transneft":
            return "Газпром Транснефть"
        if self._platform_key == "gpb_inter_rao":
            return "Газпром Интер РАО"
    ...
    def _set_platform_buttons(self) -> None:
        ...
        self.btn_platform_tektorg.setText(
            self._platform_title()
        ...
        self.act_platform_transneft.setCheckable(True)
        self.act_platform_gpb_inter_rao.setCheckable(True)
        ...
        self.act_platform_roseltorg_interrao.setCheckable(True)
        ...
        self.act_platform_transneft.setChecked(self._platform_key == "transneft")
        self.act_platform_gpb_inter_rao.setChecked(self._platform_key == "gpb_inter_rao")
        ...
        self.act_platform_roseltorg_interrao.setChecked(self._platform_key == "roseltorg_interrao")
    ...
    def _apply_platform_ui(self) -> None:
        ...
        if self._platform_key == "roseltorg":
            ...
            self.status_msg.setText("Готов. Нажмите «Поиск» и войдите через ЭЦП при необходимости.")
        elif self._platform_key == "roseltorg_223":
            self.sidebar.set_platform_filter_options(
                ROSELTORG_223_PROCEDURE_TYPE_OPTIONS,
                ROSELTORG_223_STATUS_OPTIONS,
                None,
            ...
            self.title_label.setText("Росэлторг 223-ФЗ — Актуальные процедуры")
            self.subtitle_label.setText("Поиск, фильтры, карточки и документы")
            self.lbl_counter.setText("Данных нет. Нажмите «Поиск». Если сессии нет, войдите через ЭЦП.")
            self.user_label.setText("Пользователь: —")
            self._set_badge("idle", "○  Росэлторг 223-ФЗ")
            self.status_msg.setText("Готов. Нажмите «Поиск» и войдите через ЭЦП при необходимости.")
        elif self._platform_key == "roseltorg_interrao":
            self.sidebar.set_platform_filter_options(
                ROSELTORG_INTERRAO_PROCEDURE_TYPE_OPTIONS,
                ROSELTORG_INTERRAO_STATUS_OPTIONS,
                None,
            ...
            self.title_label.setText("Росэлторг — Корпоративные закупки")
            self.subtitle_label.setText("Поиск, фильтры, карточки и документы")
            self.lbl_counter.setText("Данных нет. Нажмите «Поиск». Если сессии нет, войдите через ЭЦП.")
            self.user_label.setText("Пользователь: —")
            self._set_badge("idle", "○  Росэлторг Корпоративные закупки")
            self.status_msg.setText("Готов. Нажмите «Поиск» и войдите через ЭЦП при необходимости.")
            ...
            self.title_label.setText("Торговый портал — Ценовые запросы")
            self.subtitle_label.setText("Поиск ЦЗ, фильтры и анализ карточек")
            if not self.model.rowCount():
                self.lbl_counter.setText("Данных нет. Нажмите «Поиск».")
            self._set_badge("idle", "○  Браузер не запущен")
            self.status_msg.setText("Готов. Нажмите «Поиск».")
            ...
            self._set_badge("idle", "○  Браузер не запущен")
            self.status_msg.setText("Готов. Нажмите «Поиск».")
        elif self._platform_key == "transneft":
            self.sidebar.set_platform_filter_options(
                TRANSNEFT_PROCEDURE_TYPE_OPTIONS,
                TRANSNEFT_STATUS_OPTIONS,
                None,
            ...
            self.title_label.setText("Газпром Транснефть — Актуальные процедуры")
            self.subtitle_label.setText("Поиск, фильтры, карточки и документы")
            if not self.model.rowCount():
                self.lbl_counter.setText("Данных нет. Нажмите «Поиск».")
            self._set_badge("idle", "○  Браузер не запущен")
            self.status_msg.setText("Готов. Нажмите «Поиск».")
        elif self._platform_key == "gpb_inter_rao":
            self.sidebar.set_platform_filter_options(
                GPB_INTER_RAO_PROCEDURE_TYPE_OPTIONS,
                GPB_INTER_RAO_STATUS_OPTIONS,
                None,
            ...
            self.title_label.setText("Газпром Интер РАО — Актуальные процедуры")
            self.subtitle_label.setText("Поиск, фильтры, карточки и документы")
            if not self.model.rowCount():
                self.lbl_counter.setText("Данных нет. Нажмите «Поиск».")
            self._set_badge("idle", "○  Браузер не запущен")
            ...
            self.title_label.setText('ТЭК-Торг — НК "Роснефть"')
            self.subtitle_label.setText("Поиск, карточки, анализ и документы")
            if not self.model.rowCount():
                self.lbl_counter.setText("Данных нет. Нажмите «Поиск».")
            self._set_badge("idle", "○  Браузер не запущен")
            self.status_msg.setText("Готов. Нажмите «Поиск».")
        elif self._platform_key == "tektorg_inter_rao":
            self.sidebar.set_platform_filter_options(
                TEKTORG_INTER_RAO_TYPE_OPTIONS,
                TEKTORG_INTER_RAO_STATUS_OPTIONS,
                None,
            ...
            self.title_label.setText('ТЭК-Торг — ПАО "Интер РАО"')
            self.subtitle_label.setText("Поиск, карточки, анализ и документы")
            if not self.model.rowCount():
                self.lbl_counter.setText("Данных нет. Нажмите «Поиск».")
            self._set_badge("idle", "○  Браузер не запущен")
            self.status_msg.setText("Готов. Нажмите «Поиск».")
        elif self._platform_key == "tektorg_223":
            self.sidebar.set_platform_filter_options(
                TEKTORG_223_TYPE_OPTIONS,
                TEKTORG_223_STATUS_OPTIONS,
                None,
            ...
            self.title_label.setText("ТЭК-Торг — 223-ФЗ")
            self.subtitle_label.setText("Поиск, карточки, анализ и документы")
            if not self.model.rowCount():
                self.lbl_counter.setText("Данных нет. Нажмите «Поиск».")
            self._set_badge("idle", "○  Браузер не запущен")
    ...
    def _select_platform(self, key: str) -> None:
        ...
        if key == "roseltorg":
        ...
        elif key == "roseltorg_223":
            self.client = Roseltorg223Client()
        elif key == "roseltorg_interrao":
            self.client = RoseltorgInterRaoClient()
            ...
            self.client = TransneftClient()
        elif key == "gpb_inter_rao":
            self.client = GpbInterRaoClient()
    ...
    def _display_filters_for_platform(self, filters: ClientFilters) -> ClientFilters:
        if self._platform_key not in {"roseltorg", "roseltorg_44", "roseltorg_223", "roseltorg_interrao"}:
            return filters
    ...
    def _update_controls(self) -> None:
        ...
        self.btn_platform_tektorg.setEnabled(not running)
