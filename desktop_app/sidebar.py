...
class Sidebar(QWidget):
    ...
    def _quick_filter_box(self, label: str, widget: QWidget, min_width: int = 170) -> QWidget:
        box = QWidget()
    ...
    def _section_card(self, title: str | None) -> tuple[QWidget, QHBoxLayout]:
        card = QWidget()
        card.setObjectName("FilterSection")
    ...
    def _build_ui(self) -> None:
        ...
        toolbar = QFrame()
        toolbar.setObjectName("FiltersToolbar")
        ...
        self.btn_edit_keywords.setMaximumHeight(42)
        actions_row.addWidget(self.btn_edit_keywords)
        ...
        self.btn_reset.setMaximumHeight(44)
        ...
        self.btn_search.setMaximumHeight(44)
        actions_row.addWidget(self.btn_search)
        toolbar_layout.addLayout(actions_row)
        self.refresh_keywords_count()

        active_wrap = QWidget()
        active_wrap.setObjectName("ActiveFiltersWrap")
        self.active_filters_row = QHBoxLayout(active_wrap)
        self.active_filters_row.setContentsMargins(0, 4, 0, 0)
        self.active_filters_row.setSpacing(8)
        ...
        toolbar_layout.addWidget(active_wrap)
    ...
    def _expanded_min_height(self) -> int:
        if self._platform_key in {"roseltorg", "roseltorg_44", "roseltorg_223", "roseltorg_interrao"}:
            return 360
    ...
    def _apply_platform_filter_visibility(self) -> None:
        if self._platform_key == "roseltorg":
            ...
            self.extra_scroll.setMinimumHeight(0)
            self.extra_scroll.setMaximumHeight(16777215)
        elif self._platform_key in {"roseltorg_223", "roseltorg_interrao"}:
            visible_keys = {
                "registry",
                "trend",
                "status",
            ...
            self._set_row_label("registry", "Номер:")
            self._set_row_label("trend", "Тип процедуры ЭТП:")
            self._set_row_label("status", "Статус лота:")
            self._set_row_label("customer", "Заказчик:")
            self._set_row_label("customer_agent", "ИНН заказчика:")
            self._set_row_label("organizer", "Организатор:")
            self._set_row_label("responsible", "Контактное лицо:")
            self._set_row_label("price", "Сумма НМЦ по лотам:")
            self._set_row_label("published", "Публикация:")
            self._set_row_label("end", "Приём заявок до:")
            for row in range(40):
                self._filter_grid.setRowMinimumHeight(row, 0)
            for key in self._filter_rows:
                self._set_row_visible(key, False)
            ...
            self.extra_scroll.setMinimumHeight(0)
            self.extra_scroll.setMaximumHeight(16777215)
        elif self._platform_key in {"tektorg_rosneft", "tektorg_inter_rao", "tektorg_223", "tektorg_market", "rts_tender"}:
            visible_keys = {
                "registry",
                "trend",
                "organizer",
            ...
            self._set_row_label("registry", "Номер процедуры:")
            self._set_row_label("title", "Наименование процедуры:")
            self._set_row_label("trend", "Способ определения поставщика:")
            self._set_row_label("organizer", "Организатор:")
            self._set_row_label("status", "Статусы процедур (стадии):")
            self._set_row_label("price", "Цена:")
            self._set_row_label("published", "Публикация процедуры:")
            self._set_row_label("end", "Окончание приема заявок:")
            self._set_row_label("okpd2", "ОКПД2 (для лота):")
            self._set_row_label("okved2", "ОКВЭД2 (для лота):")
            self._set_row_label("special_features", "МСП:")
            self._set_row_label("guarantee", "Обеспечение:")
            self._set_row_label("customer_region", "Регион организатора:")
            for row in range(40):
                self._filter_grid.setRowMinimumHeight(row, 0)
            for key in self._filter_rows:
                self._set_row_visible(key, False)
