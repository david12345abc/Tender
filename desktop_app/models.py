...
class ProcedureTableModel(QAbstractTableModel):
    ...
    def _display(self, proc: dict[str, Any], key: str) -> Any:
        ...
        if key == "total_price":
            raw = proc.get("total_price")
            p = parse_price(raw)
            if p is None and raw not in (None, ""):
                return str(raw)
            return fmt_money(p, proc.get("currency_name") or "RUB")
        if key == "total_price_with_vat":
            raw = proc.get("total_price_with_vat")
            p = parse_price(raw)
            if p is None and raw not in (None, ""):
                return str(raw)
            return fmt_money(p, proc.get("currency_name") or "RUB")
    ...
    def _sort_value(self, proc: dict[str, Any], key: str) -> Any:
        if key in ("total_price", "total_price_with_vat"):
            return parse_price(proc.get(key)) or 0.0
        if key == "applics_count":
    ...
    def data(self, index: QModelIndex, role: int = Qt.DisplayRole) -> Any:
        ...
        if role == Qt.ToolTipRole:
            ...
            if col_key == "quick_actions":
                return (
                    "Первая иконка: подготовка документов (скачать, после заполнения — «Готово» и "
                    "распределение по папкам без перехода к странице заявки).\n"
                    "По центру: только хвост потока — открыть страницу подачи заявки из папки «Технические» "
                    "и робот до конца (без автоматического скачивания и распределения).\n"
                    "Справа: сценарий 3 (сообщение)."
                )
...
class ProcedureFilterProxy(QSortFilterProxyModel):
    ...
    def filterAcceptsRow(self, source_row: int, source_parent: QModelIndex) -> bool:
        ...
        if f.quick_search:
            ...
            title_blob = self._blob(
                proc,
                ("title", "name", "procedure_name", "lot_name"),
            ).casefold()
            if needle not in title_blob:
                return False
