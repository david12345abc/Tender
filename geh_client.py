...
GEH_ORGANIZATION_ID = "5ec50776-63f0-41ff-87a1-6cd125f38e78"
GEH_URL = (
    "https://geh.etpgpb.ru/"
    f"?organizationId={GEH_ORGANIZATION_ID}"
    "#com/procedure/index/223"
)

# Значения проверены на geh.etpgpb.ru через Procedure.list: поле `status` в теле RPC.
GEH_STATUS_OPTIONS = [
    ("Активные", "-2"),
    ("Прием заявок", "2"),
    ("Вскрытие заявок", "3"),
    ...
    ("Архив", "8"),
    ("Отменен", "10"),
    ("Рассмотрение вторых частей заявок", "26"),
]

GEH_STATUS_LABEL_BY_CODE = {
    -2: "Активные",
    2: "Прием заявок",
    3: "Вскрытие заявок",
    ...
    8: "Архив",
    10: "Отменен",
    26: "Рассмотрение вторых частей заявок",
}

GEH_PROCEDURE_TYPE_OPTIONS = GPB_BUSINESS_PROCEDURE_TYPE_OPTIONS


class GehClient(GpbBusinessClient):
    """Клиент секции «Газпром энергохолд» на geh.etpgpb.ru."""

    platform_key = "geh"

    def __init__(self, port: int = 9222) -> None:
        super().__init__(port=port)
        self.target_url = GEH_URL
        self.target_host = "geh.etpgpb.ru"
        ...
        if isinstance(procedures, list):
            for proc in procedures:
                ...
                if status_code is not None:
                    label = GEH_STATUS_LABEL_BY_CODE.get(status_code)
                    if label:
